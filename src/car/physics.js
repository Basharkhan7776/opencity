...
export class Car {
  constructor(track, { palette = 0, ai = false, perf = {} } = {}) {
    this.track = track;
    this.ai = ai;
  ...
  /** Swap the driveline's tuning for a different vehicle without rebuilding. */
  setPerf(perf = {}) {
  ...
  step(dt, input) {
    ...
    /* On the playground there is no loose edge — grip is full everywhere. */
    this.offRoad = this.track.freeRoam
      ? 0
    ...
    if (grounded) {
      const drive = torqueAt(this.rpm / MAX_RPM) * DRIVE_TORQUE
        * this.perf.power * GEARS[this.gear] * FINAL / wheelRadius;
    ...
    /* Drag and rolling resistance. Terminal speed lands around 195 km/h at
       perf { power:1, drag:1 }; the per-vehicle drag scales it. */
    Fx -= AERO_DRAG * this.perf.drag * this.vx * Math.abs(this.vx);
   ...
   */
  _walls(f, dt) {
    /* A free-roam world (the flat playground) has no kerbs and no berms:
       the whole plane is drivable tarmac. Skip containment entirely. */
    if (this.track.freeRoam) { this._contact = false; return; }
