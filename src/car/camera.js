...
export class ChaseCamera {
  constructor(camera) {
    ...
    this.yaw = 0;
    this.yawLag = 0;
    /* Mouse orbit around the car, radians, eased toward the requested angle
       each frame so a mouse nudge reads as a swing rather than a snap. Both
       default to zero and nothing in the rally build passes them, so the
       rally camera is untouched. */
    this.orbitYaw = 0;
    this.orbitPitch = 0;
  ...
  update(car, dt, { lookBack = false, orbitYaw = 0, orbitPitch = 0 } = {}) {
    ...
    /* Mouse orbit, on top of everything the boom already does. Yaw turns the
       boom around the car; pitch swings the lens up and down about the same
       centre, so the car stays framed while the angle changes. Eased rather
       than applied, so the rotation is a move instead of a cut. */
